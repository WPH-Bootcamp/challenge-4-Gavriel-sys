...
function addTodo() {
  ...
  const input = prompt("Enter new to-do text: ");

  // Rapikan: buang spasi di awal/akhir
  const text = input.trim();

  // 2. Validasi input: Pastikan teks tidak kosong atau hanya spasi
  if (text === "") {
    console.log("The to-do list must not be empty. Nothing has been added.");
    return;
  }
  ...
  console.log(`To-do "${text}" successfully added.`);
  console.log("Fill in all the fields now:", todos);
}

function markTodoCompleted() {
  // TODO: Implementasi logika untuk menandai to-do sebagai selesai
  // Kalau belum ada to-do sama sekali
  if (todos.length === 0) {
    console.log("There are no to-dos that can be completed yet.");
    return;
  }
  ...
  const input = prompt("Enter the to-do NUMBER you want to mark as complete: ");
  ...
  // Validasi: harus angka bulat dan dalam jangkauan
  if (!Number.isInteger(choice) || choice < 1 || choice > todos.length) {
    console.log("Invalid number. Please enter a valid number from the list.");
    return;
  }
  ...
  // Cari to-do berdasarkan nomor (ingat: index array mulai dari 0)
  const index = choice - 1;
  const todo = todos[index];
  // Cek apakah sudah selesai
  if (todo.isCompleted) {
    console.log(
      `To-do "${todo.text}" has been marked as completed previously.`
    );
    return;
  }
  // Tandai sebagai selesai
  todo.isCompleted = true;
  console.log(`To-do "${todo.text}" successfully marked as complete.`);
...
function deleteTodo() {
  ...
  // Kalau belum ada to-do sama sekali
  if (todos.length === 0) {
    console.log("There are no to-dos that can be deleted yet.");
    return;
  }
  ...
  const input = prompt("Enter the to-do NUMBER you want to delete: ");
  const trimmed = input.trim();
  ...
  // Hitung index array (user mulai dari 1, array mulai dari 0)
  const index = choice - 1;
  // 4. Hapus to-do yang dipilih dari array `todos`
  const deletedTodo = todos.splice(index, 1)[0];
  // 5. Beri feedback ke user bahwa to-do berhasil dihapus
  console.log(`To-do "${deletedTodo.text}" successfully deleted.`);
}
...
function listTodos() {
  ...
  for (let i = 0; i < todos.length; i++) {
    const todo = todos[i]; // ambil object todo di posisi ke-i
    const number = i + 1; // supaya mulai dari 1, bukan 0
...
function runTodoApp() {
  ...
  while (running) {
    console.log("\nSelect Command:");
    console.log("1. add      - Add to-do");
    console.log("2. complete - Mark to-do as completed");
    console.log("3. delete   - Delete to-do");
    console.log("4. list     - See all to-dos");
    console.log("5. exit     - Exit the application");

    const input = prompt(
      "Enter the command (add/complete/delete/list/exit or 1-5): "
    );
    ...
    if (command === "add" || command === "1") {
      addTodo();
    } else if (command === "complete" || command === "2") {
      markTodoCompleted();
      ...
      console.log("Exit the application. See you later!");
      running = false;
      // Supaya saat di-test pakai Jest, process.exit tidak benar-benar menutup test
      if (
